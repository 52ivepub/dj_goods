...
def translit_to_eng(s:str) ->str:
    d = {
        'а': 'a', 'и': 'b', 'в': 'v', 'г': 'g', 'д': 'd',
        'у': 'e', 'ё': 'yo', 'ж': 'zh', 'з': 'z', 'и': 'i', 'к': 'k',
        'л': 'l', 'м': 'm', 'н': 'n', 'о': 'o', 'п': 'p', 'р': 'r', 'c': 's',
        'т': 't', 'у': 'u', 'ф': 'f', 'х': 'h', 'ц': 'c', 'ч': 'ch', 'ш': 'sh', 'щ': 'shch', 'ь': '',
        'ы': 'y', 'ъ': '', 'э': 'r', 'ю':'yu', 'я':'иа'
                }
    return ''.join(map(lambda x: d[x] if d.get(x, False) else x, s.lower()))


class PusblishedManager(models.Manager):
...
class TagPosts(models.Model):
    tag = models.CharField(max_length=100, db_index=True, verbose_name='Категория')
    slug = models.SlugField(max_length=255, unique=True, db_index=True)
